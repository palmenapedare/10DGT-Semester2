...
def user(usr):
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        conn.close()
        return redirect(url_for('myflights', passenger_id=passenger_id))
    else:
